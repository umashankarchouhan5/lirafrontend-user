    ...
    <Popup :value="showModal"
        ...
        <div :class="$style.confirmation">
            <div >
                ...
                <p>All Amounts are in {{currency}}</p>
    ...
    methods:{
        confirmPayment(){
            const Data = {
                ...
                reference_id: this.value.reference_number,
                invoice_idstring: this.value.invoice_idstring
            }
            PaymentConfirmation(Data).then(
                (res) =>{
                    localStorage.setItem('pathToReturn',this.$route.fullPath);
                    localStorage.setItem('invoice_idstring',this.value.invoice_idstring);
                    localStorage.setItem('tblCCBeforePosting_id',res.tblCCBeforePosting_id);
                    localStorage.setItem('cc_process_id', this.processId);
        ...
        async saveLocalData(){
            ...
            localStorage.setItem('tblCCBeforePosting_id',this.res.tblCCBeforePosting_id);
            localStorage.setItem('cc_process_id', this.processId);
            return true;
        }
    }
