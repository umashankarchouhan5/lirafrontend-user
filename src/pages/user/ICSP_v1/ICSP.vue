<template>
    <FormWrapper>
        <PageTitle title="ICSP Application Form" slot="title" :link="`/help/700/${activeStep.id}`" :pdf="generatePdf" />
        <Wizard :steps="steps" @changeStep="setActiveStep"/>
        <ValidationObserver ref="validator" v-if="icsp_id !== null">
            ...
            <div :class="$style.success">
                <template v-if="form.data && form.data.TicketReference">
                    ...
                    <template v-if="modals.success.step === 3">
                        ...
                        <!-- <a @click="() => {pqDataVisible = true}">Show Person</a> -->
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.setSteps(steps);
            createNewForm({
                process_id:  APPLICATION_OF_ICSP
            }).then((response) => {
                var userData = JSON.parse(localStorage.getItem('userData')); // fetch user data from localstorage
                // userData.icsp_id = response.data[0].TAskReference_id; // mutate data
                this.icsp_id = response.data[0].TAskReference_id;
                this.$store.commit('setCspId',this.icsp_id);
        ...
        methods: {
            generatePdf(){
                ...
                formData.append('icsp_id',this.icsp_id);
                formData.append('process_id',APPLICATION_OF_ICSP);
                formData.append('prefix','ICSP');
                generateICSPPdf(formData).then(
                    res => {
            ...
            responseHandler(response) {
                ...
                if(!this.$route.params.id || (this.$route.params.id && this.$route.params.id != this.form.data.reference_id)){
                    this.$router.replace({name:'icspFormRef',params:{id:this.form.data.reference_id}});
                }
                ...
                this.$store.commit('setProcessId', APPLICATION_OF_ICSP)
                this.fetchPerson();
                this.getDeclaration();
            },
            fetchPerson(){
