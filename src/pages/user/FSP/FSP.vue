<template>
    <FormWrapper >
        <PageTitle title="FSP Application Form" slot="title" :link="`/help/701/${activeStep.id}`" :pdf="generatePdf" />
        <Wizard :steps="steps" @changeStep="setActiveStep"/>
        <ValidationObserver ref="validator" v-if="fsp_id !== null">
...
<script>
    ...
    import { APPLICATION_OF_FSP  } from 'Config/processIds';
    ...
    export default {
        ...
        created() {
            ...
            this.setSteps(steps);
            createNewForm({
                process_id:  APPLICATION_OF_FSP
            }).then((response) => {
                var userData = JSON.parse(localStorage.getItem('userData'));
                this.fsp_id = response.data[0].TAskReference_id;
                this.$store.commit('setCspId',this.fsp_id);
                userData.webUserTask_id = response.data[0].WebUserTask_id; 
        ...
        methods: {
            generatePdf(){
                ...
                formData.append('fsp_id',this.fsp_id);
                formData.append('process_id',APPLICATION_OF_FSP);
                formData.append('prefix','FSP');
                generateFSPPdf(formData).then(
                    res => {
            ...
            responseHandler(response) {
                ...
                if(!this.$route.params.id || (this.$route.params.id && this.$route.params.id != this.form.data.reference_id)){
                    this.$router.replace({name:'fspFormRef',params:{id:this.form.data.reference_id}})
                }
                ...
                this.$store.commit('setProcessId', APPLICATION_OF_FSP)
                this.fetchPerson();
                this.getDeclaration();
            },
            fetchPerson(){
