<template>
    <FormWrapper>
        <PageTitle title="My Personal Questionnaire Form" v-if="!checkIfPQExist && pqAuth.disabled" :pdf="generatePdf" slot="title" />
        <PageTitle title="My Personal Questionnaire Form" v-if="checkIfPQExist || !pqAuth.disabled" :link="`/help/${currentProcessId}`" slot="title" />
        ...
        <Popup :value="!pqAuth.disabled" :title="pqAuth.title" type="info" :closable="false" :mask-closable="false">
            ...
            <PQPassword v-if="!pqAuth.disabled" @pqValidation="checkPassword" :backButton="true"  @forgotClicked="onForgot" @resetScreen="onResetScreen" @backTo="backToList" :uniqueRef="uniqueRef" />
    ...
    export default {
        ...
        watch: {
            'pqAuth.disabled': function(val){
                if(val === true && this.uniqueRef !== null ){
                    this.getPqData();
                }
            }
        },
        ...
        methods: { 
            getPQDetails() {
                getMyPq().then(res => {
                    if(res.data.id !== undefined && res.data.id !== null) {
                        this.uniqueRef = res.data.UniqueRef;
                        if(this.$route.name !== "MyPQByID"){
                            this.$router.replace({name:'MyPQByID', params:{id: res.data.id}})
                        }
            ...
            createNewPQ(data) {
                const { Fname, Lname, DOB, PassportNo, email, salutation, my_pq } = data;
                ...
                const request = {
                    Fname, Lname, DOB, PassportNo, email, salutation, my_pq
                }
                createNewForm({
