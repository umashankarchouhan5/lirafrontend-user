<template>
    <FormWrapper>
        <PageTitle title="Changes Security" v-if="processId ===  null || processId === 0" slot="title" link="/help/2041"/>
        <PageTitle :title="`Changes Security ${processTitleName}`" v-else slot="title" :link="`/help/${processId}/${activeStep.id}`"/>
        ...
        methods: {
            ...
            setMode() {
                if(this.$route.params.id && this.$route.params.id != 'add' && this.$route.params.id != 'edit'){
                    ...
                    return this.mode = this.$route.path === '/changesSec/edit' ? 'edit' : 'add'
                }
            },
            getPcWebData(){
                getPcWeb({
            ...
            getTicketHandlerSuccess(response) {
                ...
                this.ticket.bankerJson = this.ticket.Banker.length !== 0 ? JSON.parse(this.ticket.Banker) : null;
                this.ticket.license_id = this.ticket.License_id;
                this.ticket.effectiveDate = this.ticket.EffectiveDate;
                this.ticket.tradeName = this.ticket.TradeName;
                this.isTicket = true;
                this.getPcWebData();
                // this.ticketValue = {
            ...
            saveHandler() {
                ...
                if(process_id == 2043){
                    ...
                    const data = {
                        ...
                        reference_id,
                        license_id,
                        bankerJson: JSON.stringify(bankerJson),
                        effectiveDate,
                        icsp_id: this.userData.icsp_id
                    };
                    changeTicket(data).then(this.saveTicketHandlerSuccess)
            ...
            backToList() {
                ...
                this.$router.push({ path: '/changesSec' })
            },
            resetActivatedSteps(){   
