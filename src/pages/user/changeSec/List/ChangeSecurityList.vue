<template>
    <FormWrapper>
        <PageTitle title="Change Request" slot="title" :link="`/help/${processId}?m=34`"/>
...
<script>
    ...
    import { CHANGE_SEC_PROCESS_ID } from 'Config/processIds';
    ...
    export default {
        ...
        methods: {
            setProcessId() {
                this.$store.commit('setProcessId', CHANGE_SEC_PROCESS_ID)
            },
            respondChange() {
                if (this.validateSelectedLength(1) && this.validateCanRespond()) {
                    ...
                    this.$router.push({ path: `changesSec/${reference_id}/respond/${process_id}`})
                }
            },
            validateCanRespond() {
            ...
            addChange() {
                this.$router.push({ path: '/changesSec/add' })
            },
			editChange() {
                if (this.validateSelectedLength(1) && this.validateCanEdit()) {
                    ...
                    sessionStorage.setItem(PROCESS_ID, Process_id);
                    sessionStorage.setItem(REFERENCE_ID, reference_id);
                    this.$router.push({ name: 'ChangesSecurityEdit'})
                }
            },
            pageChange(page) {
            ...
            getDataByFilter() {
                ...
                const data = {
                    filter: [{
                        ...
                        processtype_id: CHANGE_SEC_PROCESS_ID
                    }],
                    sort: [{ key, order }],
                    paging: [
                        {
			...
			getData() {
                Promise.all([
                    getStatuses({ process_id: CHANGE_SEC_PROCESS_ID }),
            ...
            parseUrlForValues() {
                ...
                if (!query || !query.status_id) {
                    this.$router.replace({query:{status_id: this.selectedTable}})
                }
            },
            handleResponse(res) {
