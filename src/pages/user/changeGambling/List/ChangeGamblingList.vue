<template>
    <FormWrapper>
        <PageTitle title="Change Request" slot="title" :link="`/help/${processId}?m=35`"/>
...
<script>
    ...
    import { CHANGE_GAM_PROCESS_ID } from 'Config/processIds';
    ...
    export default {
        ...
        methods: {
            setProcessId() {
                this.$store.commit('setProcessId', CHANGE_GAM_PROCESS_ID)
            },
            respondChange() {
                if (this.validateSelectedLength(1) && this.validateCanRespond()) {
                    ...
                    this.$router.push({ path: `changesGam/${reference_id}/respond/${process_id}`})
                }
            },
            validateCanRespond() {
            ...
            addChange() {
                this.$router.push({ path: '/changesGam/add' })
            },
			editChange() {
                if (this.validateSelectedLength(1) && this.validateCanEdit()) {
                    ...
                    sessionStorage.setItem(PROCESS_ID, Process_id);
                    sessionStorage.setItem(REFERENCE_ID, reference_id);
                    this.$router.push({ name: 'ChangesGamblingEdit'})
                }
            },
            pageChange(page) {
            ...
            getDataByFilter() {
                ...
                const data = {
                    filter: [{
                        ...
                        processtype_id: CHANGE_GAM_PROCESS_ID
                    }],
                    sort: [{ key, order }],
                    paging: [
                        {
			...
			getData() {
                Promise.all([
                    getStatuses({ process_id: CHANGE_GAM_PROCESS_ID }),
