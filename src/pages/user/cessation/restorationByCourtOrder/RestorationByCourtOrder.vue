<template>
    <FormWrapper>
        <PageTitle title="Restoration By Court Order" slot="title" :link="`/help/470`" />
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.$store.commit('setProcessId', RESTORATION_BY_COURT_ORDER);
            this.getDeclaration();
        ...
        methods:{
            ...
            saveHandler(nextStep) {
                ...
                if(data !== null){
                    cessationRequest(data).then(res=>{
                        ...
                        if(!this.$route.params.id){
                            this.$router.replace(`restorationByCourtOrder/${res.data[0].reference_id}`);
                        }
                        if(nextStep){
            ...
            getDataToSend(nextStep){
                if( this.form.data.company_id && this.form.data.company_id !== null && this.form.data.company_id !== ''){
                    ...
                    const { company_id, effectiveDate, cessationReason, solvent } = this.form.data;
                    const data = {
                        process_id: RESTORATION_BY_COURT_ORDER,
                        reference_id: this.reference_id,
                        company_id, 
