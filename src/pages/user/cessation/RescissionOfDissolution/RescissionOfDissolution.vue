<template>
    <FormWrapper>
        <PageTitle title="Rescission of Dissolution" slot="title" :link="`/help/470`" />
...
<script>
    ...
    import { RESCISSION_OF_DISSOLUTION  } from 'Config/processIds';
    ...
    export default {
        ...
        created() {
            ...
            this.$store.commit('setProcessId', RESCISSION_OF_DISSOLUTION);
            this.getDeclaration();
        ...
        methods:{
            getData(){
                getTicket({
                    process_id:  RESCISSION_OF_DISSOLUTION,
            ...
            saveHandler(nextStep) {
                ...
                if(data !== null){
                    cessationRequest(data).then(res=>{
                        ...
                        if(!this.$route.params.id){
                            this.$router.replace(`rescissionOfDissolution/${res.data[0].reference_id}`);
                        }
                        if(nextStep){
            ...
            getDataToSend(nextStep){
                if( this.form.data.company_id && this.form.data.company_id !== null && this.form.data.company_id !== ''){
                    ...
                    const { company_id, effectiveDate, liquidatorName, liquidatorAddress } = this.form.data;
                    const data = {
                        process_id: RESCISSION_OF_DISSOLUTION,
                        reference_id: this.reference_id,
                        company_id, 
