<template>
    <FormWrapper>
        <PageTitle title="DeRegistration of Trust" slot="title" :link="`/help/470`" />
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.$store.commit('setProcessId', DEREGISTRATION_OF_TRUST);
            this.getDeclaration();
            ...
            saveHandler(nextStep) {
                ...
                if(data !== null){
                    cessationRequest(data).then(res=>{
                        ...
                        if(!this.$route.params.id){
                            this.$router.replace(`deregistrationOfTrust/${res.data[0].reference_id}`);
                        }
                        if(nextStep){
