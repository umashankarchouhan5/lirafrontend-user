<template>
    <div>
        ...
        <FormRow >
            <div class="col-sm-3">
                <InputDate rules="required" :readonly="readonly" label="Effective Date" :options="options"  v-model="innerValue.effectiveDate"  />
            </div>
        </FormRow>
<!--        
        <FormRow>
        ...
        <!-- <FormRow >
            <div class="col-sm-12">
                <InputTextArea :readonly="readonly" label="Reason" rows="5" v-model="innerValue.cessationReason"  />
            </div>
        </FormRow> -->
    </div>
</template>
...
<script>
    ...
    import { CESSATION_OUTSIDE_SEYCHELLES  } from 'Config/processIds';
    ...
    export default {
        ...
        created() {
            ...
            if(this.innerValue.company_id !== null && this.$route.params.id ){
        ...
        watch: {
            innerValue(val) {
                if(val.company_id !== null && this.$route.params.id ){
                    this.company.id = val.company_id;
                    this.company.Name = val.companyName;
                    this.company.EntityType = val.entityType;
                    this.company.CompanyRegNo = val.companyRegNo;
                    this.$store.commit('setCompany',this.company);
                }
            },
            'company.EntityType': function(val){
                this.innerValue.EntityType = val;
                if(val !== null && val !== undefined){
                    this.entitySelected = true;
                    this.$emit('onChangeSteps',val);
                }else{
        ...
        methods: {
            getData(){
                Promise.all([
                    getStrikeOffReason({process_id: CESSATION_OUTSIDE_SEYCHELLES})
            ...
            getEntityListHandler() {
                getEntityType({
                    process_id: CESSATION_OUTSIDE_SEYCHELLES,
