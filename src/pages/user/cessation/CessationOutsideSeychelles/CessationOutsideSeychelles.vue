<template>
    <FormWrapper>
        <PageTitle title="Continuation Outside Seychelles" slot="title" :link="`/help/106`" />
...
<script>
    ...
    export default {
        name: 'ContinuationOutsideSeychelles',
        ...
        created() {
            this.setSteps(steps['IBC']);
            if(this.$route.params.id){
                this.reference_id = parseInt(this.$route.params.id);
                this.getData();
            }
            this.$store.commit('setProcessId', CESSATION_OUTSIDE_SEYCHELLES);
            this.getDeclaration();
        ...
        methods:{
            changeSteps(val) {
                if(val == 'Foundation'){
                    this.setSteps(steps['Foundation']);
                }
                else{
                    this.setSteps(steps['IBC']);
                }
            ...
            responseHandler(data){
                ...
                if(this.form.data.EntityType == 'Foundation'){
                    this.setSteps(steps['Foundation'])
                }
            ...
            saveHandler(nextStep) {
                ...
                if(data !== null){
                    cessationRequest(data).then(res=>{
                        ...
                        if(!this.$route.params.id){
                            this.$router.replace(`cessationOutsideSeychelles/${res.data[0].reference_id}`);
                        }
                        if(nextStep){
            ...
            getDataToSend(nextStep){
                if( this.form.data.company_id && this.form.data.company_id !== null && this.form.data.company_id !== ''){
                    ...
                    const { company_id, effectiveDate, cessationReason } = this.form.data;
                    const data = {
                        process_id: CESSATION_OUTSIDE_SEYCHELLES,
                        reference_id: this.reference_id,
                        company_id, 
