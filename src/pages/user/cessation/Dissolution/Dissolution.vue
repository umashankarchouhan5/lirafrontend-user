<template>
    <FormWrapper>
        <PageTitle title="Completion of Dissolution" slot="title" :link="`/help/470`" />
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.$store.commit('setProcessId', DISSOLUTION);
            this.getDeclaration();
        ...
        methods:{
            ...
            saveHandler(nextStep) {
                ...
                if(data !== null){
                    cessationRequest(data).then(res=>{
                        ...
                        if(!this.$route.params.id){
                            this.$router.replace(`dissolution/${res.data[0].reference_id}`);
                        }
                        if(nextStep){
            ...
            getDataToSend(nextStep){
                if( this.form.data.company_id && this.form.data.company_id !== null && this.form.data.company_id !== ''){
                    ...
                    const { company_id, effectiveDate } = this.form.data;
                    const data = {
                        process_id: DISSOLUTION,
                        reference_id: this.reference_id,
                        company_id, 
