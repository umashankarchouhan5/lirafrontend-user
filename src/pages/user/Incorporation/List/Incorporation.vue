<template>
    <FormWrapper>
        <PageTitle title="Incorporation" slot="title" :link="`/help/${processId}`"/>
        ...
        <DataTable :actions="true"
            ...
            <template slot="actionsLeft">
                <template >
                    <TableButton @click="() => goToTicket(INCORPORATE_PROCESS_ID)">
                        <img src="../../../../assets/images/incorpo.png"/>
                        Incorporate
                    </TableButton>
                </template>
                <template v-if="respondStatus === 32">
                    <TableButton @click="incorporateRespond">
...
<script>
    ...
    export default {
        ...
        methods: {
            getPCWebData() {
                getPcWeb({process_id: INCORPORATE_PROCESS_ID}).then( res=> {
            ...
            goToTicket(selectedProcessId) {
                if(this.userData.isRestricted == 1 && this.pcWeb.isRestricted == 1){
                    ...
                    if (this.validateBeforeGoToTicket(selectedProcessId)) {
                        ...
                        this.$router.push({ path: `/incorporation/${reference_id}` })
                    }
                }
            },
            ...
            setProcessId() {
                this.$store.commit('setProcessId', INCORPORATE_PROCESS_ID)
            },
            sortChange(data) {
            ...
            fetchEntityList(){
                getEntityType({ process_id: this.processId,icsp_id: this.userData.icsp_id }).then((res) => {
