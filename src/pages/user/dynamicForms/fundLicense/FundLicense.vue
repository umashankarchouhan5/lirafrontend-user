<template>
    <FormWrapper >
        <PageTitle :title="pageTitle" slot="title" :link="`/help/2000/${activeStep.id}`" />
        <div >
...
<script>
    ...
    export default {
        ...
        created() {
            this.setSteps(steps);
            fetchLicenseApplicantCompany({process_id: FUND_LICENSE}).then(
                res => {
        ...
        methods: {
            generatePdf(){
                ...
                formData.append('process_id', FUND_LICENSE);
                formData.append('prefix','FUND_LICENSE');
            ...
            generateDraft(){
                createNewForm({process_id: FUND_LICENSE, company_id: this.company.company_id}).then(
                    res => {
                        this.reference_id = res.data[0].reference_id;
                        this.$router.replace({name:"FundLicenseById",params:{id:res.data[0].reference_id}})
            ...
            getDataById(val){
                getDynamicDatabyID({process_id: FUND_LICENSE, reference_id: val}).then(
                    res => {
                        this.form = res.data[1];
                        this.pageTitle = res.data[0][0].FormTitle;
                        this.$store.commit('setProcessId', FUND_LICENSE);
                        this.getDeclaration();
