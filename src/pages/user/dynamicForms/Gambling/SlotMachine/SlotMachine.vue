...
<script>
    ...
    import { SLOT_MACHINE } from 'Config/processIds';
    ...
    export default {
        ...
        created() {
            this.setSteps(steps);
            fetchLicenseApplicantCompany({process_id: SLOT_MACHINE}).then(
                res => {
        ...
        methods: {
            generatePdf(){
                ...
                formData.append('process_id', SLOT_MACHINE);
                formData.append('prefix','SLOT_MACHINE');
            ...
            generateDraft(){
                createNewForm({process_id: SLOT_MACHINE, company_id: this.company.company_id}).then(
                    res => {
                        this.reference_id = res.data[0].reference_id;
                        this.$router.replace({name:"SlotMachineLicenseById",params:{id:res.data[0].reference_id}})
            ...
            getDataById(val){
                getDynamicDatabyID({process_id: SLOT_MACHINE, reference_id: val}).then(
                    res => {
                        this.form = res.data[1];
                        this.pageTitle = res.data[0][0].FormTitle;
                        this.$store.commit('setProcessId', SLOT_MACHINE);
                        this.getDeclaration();
