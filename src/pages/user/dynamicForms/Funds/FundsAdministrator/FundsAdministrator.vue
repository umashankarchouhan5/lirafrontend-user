<template>
    <FormWrapper >
        <PageTitle :title="pageTitle" slot="title" :link="`/help/2013/${activeStep.id}`" />
        <div >
...
<script>
    ...
    import { FUNDS_ADMINISTRATOR } from 'Config/processIds';
    ...
    export default {
        ...
        created() {
            this.setSteps(steps);
            fetchLicenseApplicantCompany({process_id: FUNDS_ADMINISTRATOR}).then(
                res => {
        ...
        methods: {
            generatePdf(){
                ...
                formData.append('process_id', FUNDS_ADMINISTRATOR);
                formData.append('prefix','FUNDS_ADMINISTRATOR');
            ...
            generateDraft(){
                createNewForm({process_id: FUNDS_ADMINISTRATOR, company_id: this.company.company_id}).then(
                    res => {
                        this.reference_id = res.data[0].reference_id;
                        this.$router.replace({name:"FundsAdministratorById",params:{id:res.data[0].reference_id}})
            ...
            getDataById(val){
                getDynamicDatabyID({process_id: FUNDS_ADMINISTRATOR, reference_id: val}).then(
                    res => {
                        this.form = res.data[1];
                        this.pageTitle = res.data[0][0].FormTitle;
                        this.$store.commit('setProcessId', FUNDS_ADMINISTRATOR);
                        this.getDeclaration();
