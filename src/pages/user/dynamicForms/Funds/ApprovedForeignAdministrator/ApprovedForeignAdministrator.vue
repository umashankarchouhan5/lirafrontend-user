<template>
    <FormWrapper >
        <PageTitle :title="pageTitle" slot="title" :link="`/help/2011/${activeStep.id}`" />
        <div >
...
<script>
    ...
    import { APPROVED_FOREIGN_ADMINISTRATOR } from 'Config/processIds';
    ...
    export default {
        ...
        created() {
            this.setSteps(steps);
            fetchLicenseApplicantCompany({process_id: APPROVED_FOREIGN_ADMINISTRATOR}).then(
                res => {
        ...
        methods: {
            generatePdf(){
                ...
                formData.append('process_id', APPROVED_FOREIGN_ADMINISTRATOR);
                formData.append('prefix','APPROVED_FOREIGN_ADMINISTRATOR');
            ...
            generateDraft(){
                createNewForm({process_id: APPROVED_FOREIGN_ADMINISTRATOR, company_id: this.company.company_id}).then(
                    res => {
                        this.reference_id = res.data[0].reference_id;
                        this.$router.replace({name:"ForeignAdminById",params:{id:res.data[0].reference_id}})
            ...
            getDataById(val){
                getDynamicDatabyID({process_id: APPROVED_FOREIGN_ADMINISTRATOR, reference_id: val}).then(
                    res => {
                        this.form = res.data[1];
                        this.pageTitle = res.data[0][0].FormTitle;
                        this.$store.commit('setProcessId', APPROVED_FOREIGN_ADMINISTRATOR);
                        this.getDeclaration();
