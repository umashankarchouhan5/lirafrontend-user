...
<script>
    ...
    import { DOMESTIC_INSURER } from 'Config/processIds';
    ...
    export default {
        ...
        created() {
            this.setSteps(steps);
            fetchLicenseApplicantCompany({process_id: DOMESTIC_INSURER}).then(
                res => {
        ...
        methods: {
            ...
            generatePdf(){
                ...
                formData.append('process_id', DOMESTIC_INSURER);
                formData.append('prefix','DOMESTIC_INSURER');
            ...
            generateDraft(){
                createNewForm({process_id: DOMESTIC_INSURER, company_id: this.company.company_id}).then(
                    res => {
                        this.reference_id = res.data[0].reference_id;
                        this.$router.replace({name:"DomesticInsurerById",params:{id:res.data[0].reference_id}})
            ...
            getDataById(val){
                getDynamicDatabyID({process_id: DOMESTIC_INSURER, reference_id: val}).then(
                    res => {
                        this.form = res.data[1];
                        this.pageTitle = res.data[0][0].FormTitle;
                        this.$store.commit('setProcessId', DOMESTIC_INSURER);
                        this.getDeclaration();
