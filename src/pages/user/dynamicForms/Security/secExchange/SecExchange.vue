<template>
    <FormWrapper >
        <PageTitle :title="pageTitle" :pdf="generatePdf" slot="title" :link="`/help/2005/${activeStep.id}`"  />
        <div >
...
<script>
    ...
    import { SECURITY_EXCHANGE } from 'Config/processIds';
    ...
    export default {
        ...
        created() {
            this.setSteps(steps);
            fetchLicenseApplicantCompany({process_id: SECURITY_EXCHANGE}).then(
                res => {
        ...
        methods: {
            generatePdf(){
                ...
                formData.append('process_id',SECURITY_EXCHANGE);
                formData.append('prefix','SECURITY_EXCHANGE');
            ...
            generateDraft(){
                createNewForm({process_id: SECURITY_EXCHANGE, company_id: this.company.company_id}).then(
                    res => {
                        this.reference_id = res.data[0].reference_id;
                        this.$router.replace({name:"SecExchangeById",params:{id:res.data[0].reference_id}})
            ...
            getDataById(val){
                getDynamicDatabyID({process_id: SECURITY_EXCHANGE, reference_id: val}).then(
                    res => {
                        this.form = res.data[1];
                        this.pageTitle = res.data[0][0].FormTitle;
                        this.$store.commit('setProcessId', SECURITY_EXCHANGE);
                        this.getDeclaration();
