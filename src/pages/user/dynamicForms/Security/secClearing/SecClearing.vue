...
<script>
    ...
    import { SECURITY_CLEARING } from 'Config/processIds';
    ...
    export default {
        ...
        created() {
            this.setSteps(steps);
            fetchLicenseApplicantCompany({process_id: SECURITY_CLEARING}).then(
                res => {
        ...
        methods: {
            generatePdf(){
                ...
                formData.append('process_id',SECURITY_CLEARING);
                formData.append('prefix','SECURITY_CLEARING');
            ...
            generateDraft(){
                createNewForm({process_id: SECURITY_CLEARING, company_id: this.company.company_id}).then(
                    res => {
                        this.reference_id = res.data[0].reference_id;
                        this.$router.replace({name:"SecClearingById",params:{id:res.data[0].reference_id}})
            ...
            getDataById(val){
                getDynamicDatabyID({process_id: SECURITY_CLEARING, reference_id: val}).then(
                    res => {
                        this.form = res.data[1];
                        this.pageTitle = res.data[0][0].FormTitle;
                        this.$store.commit('setProcessId', SECURITY_CLEARING);
                        this.getDeclaration();
