<template>
    <FormWrapper >
        <PageTitle :title="pageTitle" :pdf="generatePdf" slot="title" :link="`/help/2006/${activeStep.id}`" />
        <div >
...
<script>
    ...
    import { SECURITY_FACILITY } from 'Config/processIds';
    ...
    export default {
        ...
        created() {
            this.setSteps(steps);
            fetchLicenseApplicantCompany({process_id: SECURITY_FACILITY}).then(
                res => {
        ...
        methods: {
            generatePdf(){
                ...
                formData.append('process_id',SECURITY_FACILITY);
                formData.append('prefix','SECURITY_FACILITY');
            ...
            generateDraft(){
                createNewForm({process_id: SECURITY_FACILITY, company_id: this.company.company_id}).then(
                    res => {
                        this.reference_id = res.data[0].reference_id;
                        this.$router.replace({name:"SecFacilityById",params:{id:res.data[0].reference_id}})
            ...
            getDataById(val){
                getDynamicDatabyID({process_id: SECURITY_FACILITY, reference_id: val}).then(
                    res => {
                        this.form = res.data[1];
                        this.pageTitle = res.data[0][0].FormTitle;
                        this.$store.commit('setProcessId', SECURITY_FACILITY);
                        this.getDeclaration();
