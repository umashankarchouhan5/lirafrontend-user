<template>
    <FormWrapper >
        <PageTitle :title="pageTitle" :pdf="generatePdf" slot="title" :link="`/help/2000/${activeStep.id}`"  />
        <div >
    ...
    export default {
        ...
        methods: {
            generatePdf(){
                ...
                formData.append('process_id',SECURITY_LICENSE);
                formData.append('prefix','SECURITY_LICENSE');
            ...
            generateDraft(){
                createNewForm({process_id: SECURITY_LICENSE, company_id: this.company.company_id}).then(
                    res => {
                        this.reference_id = res.data[0].reference_id;
                        this.$router.replace({name:"SecurityLicenseById",params:{id:res.data[0].reference_id}})
            ...
            getDataById(val){
                getDynamicDatabyID({process_id: SECURITY_LICENSE, reference_id: val}).then(
                    res => {
                        ...
                        this.$store.commit('setProcessId', SECURITY_LICENSE);
                        this.getDeclaration();
