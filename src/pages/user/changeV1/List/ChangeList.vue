<template>
    <FormWrapper>
        <PageTitle title="Change Request" slot="title" :link="`/help/${processId}?m=30`"/>
...
<script>
    ...
    import { getChangeList,getStatuses,getEntityType } from './config/api';
    import { PROCESS_ID, REFERENCE_ID } from 'Config/localStorageKeys';
    import config from './config/table';
    import { CHANGE_PROCESS_ID } from 'Config/processIds';
    ...
    export default {
        ...
        methods: {
            setProcessId() {
                this.$store.commit('setProcessId', CHANGE_PROCESS_ID)
            },
            respondChange() {
            ...
            addChange() {
                this.$router.push({ path: '/changes/add' })
            },
			editChange() {
			...
			getData() {
                Promise.all([
                    getStatuses({ process_id: 30 }),
                    getEntityType({ process_id: 300 })

                ]).then(this.getDataSuccess)
            },
            fetchEntityList(){
                getEntityType({ process_id: 300,icsp_id: this.userData.icsp_id }).then((res) => {
