<template>
    <FormWrapper :is-loading="isLoading">
        <PageTitle title="PCC" slot="title" link="/help/113" />
...
<script>
    ...
    import { APPLICATION_OF_PCC  } from 'Config/processIds';
    ...
    export default {
        ...
        methods: {
            ...
            editTask(){
                 if (this.validateSelectedLength(1)) {
                    ...
                    if(row.Status_id === 43){
                        ...
                        this.$router.push({name:'PCCForm',query:{web:webTaskId,task:referenceID}})
            ...
            confirmDelete(){
                const row = this.selectedRows[0];
                deleteApi({process_id: APPLICATION_OF_PCC,reference_id: row.reference_id}).then(
                    res => {
            ...
            respond(){
                if (this.validateSelectedLength(1)) {
                    const row = this.selectedRows[0];
                    if( row.Status_id === 32 ){
                        this.$store.dispatch('setProcessId', APPLICATION_OF_PCC);
            ...
            getFilterValues() {
                this.showLoader();
                Promise.all([
                    getStatuses({process_id: APPLICATION_OF_PCC}),
            ...
            getDataByFilter() {
                ...
                filters.process_id = APPLICATION_OF_PCC;
