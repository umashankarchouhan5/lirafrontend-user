<template>
    <FormWrapper>
        <PageTitle title="Surrender of License" slot="title" />
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            // }
            if( this.$route.name == 'SurrenderLicenseForm'){
                this.$store.commit('setProcessId', SURRENDER);
                this.getDeclaration();
            }
            else if( this.$route.name == 'SurrenderLicenseSecurityForm'){
                this.$store.commit('setProcessId', SURRENDER_SECURITY);
                this.getDeclaration();
            }
            else if( this.$route.name == 'SurrenderLicenseInsuranceForm'){
                this.$store.commit('setProcessId', SURRENDER_INSURANCE);
                this.getDeclaration();
            }
            else if( this.$route.name == 'SurrenderLicenseGamblingForm'){
        ...
        methods:{
            getProcessIdFormMenu(){
                ...
                //         return item.URL == "/surrenderLicense";
                //     }).process_id;
                //     return process_id;
                // }
                // else{
                //     return 410;
                // }
                if( this.$route.name == 'SurrenderLicenseSecurityForm'){
                    return SURRENDER_SECURITY;
                }
                else if( this.$route.name == 'SurrenderLicenseInsuranceForm'){
                    return SURRENDER_INSURANCE;
                }
                else if( this.$route.name == 'SurrenderLicenseGamblingForm'){
                    return SURRENDER_GAMBLING;
                }
            },
            getData(){
                getTicket({
            ...
            getDataToSend(nextStep){
                const { company_id, effectiveDate, cessationReason, license_id, measure } = this.form.data;
                const data = {
                    process_id: this.currentProcessId,
                    reference_id: this.reference_id,
                    effectiveDate, 
